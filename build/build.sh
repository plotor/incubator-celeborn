...
function build_spark_client {
  ...
  CLIENT_DIST_DIR="$DIST_DIR/spark$SPARK_MAJOR_VERSION"
  mkdir -p $CLIENT_DIST_DIR

  ## Copy spark client jars
  cp "$PROJECT_DIR"/client-spark/spark-$SPARK_MAJOR_VERSION-shaded/target/celeborn-client-spark-${SPARK_MAJOR_VERSION}-shaded_$SCALA_VERSION-$VERSION.jar $CLIENT_DIST_DIR
}
